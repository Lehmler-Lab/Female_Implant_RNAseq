---
title: "STN RNAseq Differential Expression"
output:
...
```{r}
se1<- se[,se$tissue_type == "STN"] ##change tissue type here
#se1<-se1[ , !se1$bam_ID == c("CRBM_S32.bam","CRBM_S3.bam")] #remove specific outliers here
#se1 <- se1[ rowSums(assay(se1)) >= 100, ]
se1 <- se1[ rowSums(assay(se1) >= 1) >= 8, ]
#F_se<-se1[,se1$sex == "F"]
#M_se<-se1[,se1$sex == "M"]
...
#PCA Plot
```{r}
tiff(file = 'Figures/PCA/Female_STN_PCA.tiff', units="in", width=8, height=5, res=1000)
...
sum(res_high$log2FoldChange > 1 & res_high$pvalue < 0.05, na.rm=TRUE) #upregulated genes based on pvalue instead of adj. pvalue
sum(res_high$log2FoldChange < -1 & res_high$pvalue < 0.05, na.rm=TRUE) #downregulated genes based on pvalue instead of adj. pvalue
...
```{r}
tiff(file = "Figures/heatmap/Female_STN_heatmap.tiff", units="in", width=8, height=5, res=1000)
...
#***volcano plots***
```{r}
tiff(file = 'Figures/volcano/High_volcano_plot_STN.tiff', units="in", width=8, height=5, res=1000)
...
write_tsv(High_resOrdered, file="Result_spreadsheets/High_Female_STN_results.tsv")
write_csv(High_resOrdered, file="Result_spreadsheets/High_Female_STN_results.csv")
write_csv(as.data.frame(assay(se1)), file= "Result_spreadsheets/Female_STN_counts.csv")
```


```{r}
tiff(file = 'Figures/volcano/Medium_volcano_plot_STN.tiff', units="in", width=8, height=5, res=1000)
...
write_tsv(medium_resOrdered, file="Result_spreadsheets/Medium_Female_STN_results.tsv")
write_csv(medium_resOrdered, file="Result_spreadsheets/Medium_Female_STN_results.csv")
write_csv(as.data.frame(assay(se1)), file= "Result_spreadsheets/Medium_Female_STN_counts.csv")
```


```{r}
tiff(file = 'Figures/volcano/Low_volcano_plot_STN.tiff', units="in", width=8, height=5, res=1000)
...
write_tsv(low_resOrdered, file="Result_spreadsheets/Low_Female_STN_results.tsv")
write_csv(low_resOrdered, file="Result_spreadsheets/Low_Female_STN_results.csv")
write_csv(as.data.frame(assay(se1)), file= "Result_spreadsheets/Low_Female_STN_counts.csv")
```
...
write.csv(data ,file="Figures/Enrichment/Grouped_STN_Gene_enrichment.csv")
```


```{r}
tiff(file = 'Figures/Enrichment/STN_grouped_gene_enrichment.tiff', units="in", width=8, height=5, res=1000)
...
data<-data.frame(STN_groups)
write.csv(data ,file="Figures/Enrichment/Grouped_STN_KEGG_enrichment.csv")
```


```{r}
tiff(file = 'Figures/Enrichment/STN_grouped_kegg_enrichment.tiff', units="in", width=8, height=5, res=1000)
...
data<-data.frame(STN_groups)
write.csv(data ,file="Figures/Enrichment/Grouped_STN_Disease_enrichment.csv")
```


```{r}
tiff(file = 'Figures/Enrichment/STN_grouped_disease_enrichment.tiff', units="in", width=8, height=5, res=1000)
...
tiff(file = 'Figures/STN_dot_plots_big.tiff', units="in",width=10, height=15, res=500)
...
```{r}
top_gene<-read.csv("gene_list_drug_metabolizing_kegg_striatum.csv", header = FALSE)
...
ggexport(
  plotlist = plot_list, filename = "Figures/male_STN_cyp_plot.pdf", 
